...
def load_pipeline_stage_prompt(stage: int) -> str:
    ...
    key = f'pipeline_stage{stage}_prompt_path'
    try:
        path = get_abs_path(prompts_conf[key])
    except KeyError as e:
        logger.error(f'[load_pipeline_stage_prompt]yaml中缺少配置项: {key}')
        raise e
    try:
        return open(path, 'r', encoding='utf-8').read()
    except Exception as e:
        logger.error(f'[load_pipeline_stage_prompt]读取{path}出错: {str(e)}')
        raise e
