...
无需手动切换模式：输入框统一接收问题，由**确定性意图规则**（正则，不经 LLM 判断）自动路由：

- 命中诊断/报告意图（如"诊断分析"、"病情分析"、"出具诊断报告"）→ **4 阶段流水线**，侧边栏实时显示阶段进度与中间产物
- 普通医学问答 → **单 Agent 模式**，侧边栏实时显示工具执行轨迹（检索知识库、查询病历等）

```
P1001 胸痛3天，请帮我做诊断分析    → 流水线模式
高血压的诊断标准是什么             → 单Agent模式
```
...
k: 6                    # 最终返回分片数(重排精选后)
fetch_k: 15             # 召回阶段候选数(BM25与向量各自召回后融合)
...
```bash
python tests/run_eval.py                        # 评测当前配置(混合检索+重排, 按科室过滤)
python tests/run_eval.py --compare              # 与优化前配置(朴素向量top-3)对比
python tests/run_eval.py --compare --no-filter  # 全库检索(无科室过滤), 检验开放检索下的真实增益
```

当前结果（k=6, fetch_k=15）：

| 检索范围 | 混合检索+重排 | 朴素向量 top-3（基线） |
|---|---|---|
| 按科室过滤 | **100%（30/30）** | 93.3%（28/30） |
| 全库无过滤 | **100%（30/30）** | 93.3%（28/30） |

报告自动写入 `tests/eval_report.md`（带过滤）/ `tests/eval_report_nofilter.md`（全库）。

### 消融实验（`python tests/run_ablation.py`）

为拆开"召回预算"与"检索方法"各自的贡献，全库无过滤下对四个臂对比：

| 方案 | 命中率 | 说明 |
|---|---|---|
| 向量 top-3（基线） | 93.3% | 优化前线上配置 |
| 向量 top-6 | 100% | 只扩大预算 → **预算贡献是主要增益** |
| 混合 top-6（无 rerank） | 93.3% | 只加混合召回 → BM25 拉入同文档相邻分片反而稀释目标 |
| 混合 + rerank top-6 | 100% | rerank 修复混合召回的排序稀释 → **等预算下 rerank 贡献 +6.7pp** |

诚实结论：本知识库规模下，命中率增益主要来自召回预算扩大；rerank 的可量化价值体现在
"混合召回必须配重排才不退化"，混合检索+重排的鲁棒性优势需更大型语料才能进一步显现。
调参过程：k/fetch_k 偏小（3/10 或 5/10）时全库检索漏掉 2 条关键词型用例——召回侧已含
目标分片，但 rerank 对药品名/指标类查询的排序偏差将其挤出 top-k；扩大召回预算 +
RRF 双序融合后解决。
